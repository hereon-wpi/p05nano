...
fScanParamLogFile.writelines("Scriptname: %s \n Beamtime: %s \n Prefix: %s \n RotCenter: %s \n SampleOut: %s \n Exposure Time: %s \n Speed: %s \n Smearing: %s \n Stepsize: %s \n Number of Distances: %s \n Number of Flats: %s \n CloseShutter %s" %(scriptname, beamtime, prefix, rotCenter,sampleOut, exptime , speed, smearing, stepsize,num_dist,num_flat,CS))
...
for i1 in range(num_dist):     
    ...
    time.sleep(0.1)
    nanoScript.SetCurrentName('img_y' + str(i1),iNumber =i1,imgNumber=0)
    
    nanoScript.HamaTakeTomo(target_pos)
    time.sleep(1)
    time.sleep(1 + 1/speed)
    #pmac.checkInitController5()
