...
class cPixLink_LiveImage(QtGui.QMainWindow):
    def __init__(self, parent=None, name='PixLink live image'):
        super(cPixLink_LiveImage, self).__init__()
        try:
            QtUic.loadUi('h:/_data/programming_python/p05/gui/PixLinklive_ui.ui', self)
            self.setWindowIcon(QtGui.QIcon('h:/_data/programming_python/p05/gui/pco.png'))
        except:
            _path = misc.GetPath('PixLinklive_ui.ui')
        ...
        # Tango Server for PixelLink
        self.tPixLink = PyTango.DeviceProxy('//hzgpp05vme1.desy.de:10000/p05/camera/pixlink')
        # Tango Server for Trigger
        self.tTrigger = PyTango.DeviceProxy('//hzgpp05vme1:10000/p05/dac/eh1.01')
        
        self.exptime = self.tPixLink.read_attribute('SHUTTER').value
        ...
        # connects buttons with clickButton function
    ...
    def clickButtonSetExptime(self):
        ...
        try: 
            ...
            if self.activeUpdate:
                ...
                time.sleep(0.1)
                self.t0 = time.time()
                self.tPixLink.command_inout('AbortAcq')
                while True:
                    if self.tPixLink.state() == PyTango.DevState.RUNNING: time.sleep(0.1)
                    else:
                        break
                    if time.time() - self.t0 > 60: break
                self.PollingThread.set_delay(self.exptime)
                self.tPixLink.write_attribute('SHUTTER', self.exptime)
                time.sleep(0.5)
                self.tPixLink.command_inout('StartVideoAcq')
    ...
    def clickButtonGetImageData(self):
        try:
            self.exptime = self.tPixLink.read_attribute('SHUTTER').value
            tmp = numpy.fromstring(self.tPixLink.read_attribute('Image').value, dtype=numpy.uint8).byteswap()
            self.image = (tmp).reshape(numpy.shape(self.tPixLink.read_attribute('Image').value))
            self.imagesize = numpy.shape(self.tPixLink.read_attribute('Image').value)
#             tmp = numpy.fromstring(self.tPixLink.read_attribute('Image').value, dtype=numpy.uint8).byteswap()
    ...
    def clickButtonSnapshop(self):
        try: 
            if self.activeUpdate:
                ...
                self.activeUpdate = False
                if self.tPixLink.state() == PyTango.DevState.RUNNING:
                    self.tPixLink.command_inout('AbortAcq')
    ...
    def clickButtonStartLive(self):
        if self.activeUpdate:
            ...
            if self.tPixLink.state() == PyTango.DevState.RUNNING:
                self.tPixLink.command_inout('AbortAcq')
                time.sleep(0.1)
            if self.tPixLink.state() != PyTango.DevState.ON:
                QtGui.QMessageBox.warning(self, 'Warning', 'PixLink Tango server not in on state!', buttons=QtGui.QMessageBox.Ok)
                return None
            self.tPixLink.command_inout('StartVideoAcq')
            time.sleep(2)
            self.tTrigger.write_attribute('Voltage', 0)
...
class  PixLinkpolling(QtCore.QThread):
    def __init__(self, parentThread, PixLink, trigger, cur_delay = 0.25):
        QtCore.QThread.__init__(self, parentThread)
        self.parent = parentThread
        self.Image = None
        self.tPixLink = PixLink
...
class UpdateThread(PixLinkpolling):
    def __init__(self, parentThread, PixLink, trigger, cur_delay = 0.25):
        PixLinkpolling.__init__(self, parentThread, PixLink, trigger, cur_delay = cur_delay)
        return None
            
    def updatePixLink(self):
        while True:
            if self.running:
                self.t0 = time.time()
                self.exptime = self.tPixLink.read_attribute('SHUTTER').value
...
def PixLink_LiveImage(parent=None, devices=None, groups=None, name='PixLink live image'):
    app = QtGui.QApplication(sys.argv)
    gui = cPixLink_LiveImage(name=name, parent=QtGui.QMainWindow())
    gui.PollingThread = UpdateThread(gui.main, gui.tPixLink, gui.tTrigger)
